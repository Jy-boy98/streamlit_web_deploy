...
def visualization_data():
    ...
    def preprocessing_conversion(df_before):
        ...
        # CORES 대문자로 변환
        df_after[CORES] = df_after[CORES].map(lambda x: x.upper() if isinstance(x, str) else x)
    ...
    sns.heatmap(data = df_after.corr(numeric_only=True), linewidths=0.1, annot=True, fmt ='.4f', cmap='Blues', ax = ax)
    st.pyplot(fig)
